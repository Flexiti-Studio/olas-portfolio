...
const CONFIG_NAME = 'CONFIG_GIG_WEBSITES';
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error("GET /api/gig-websites ERROR:", error);
  ...
  try {
    ...
    if (existing) {
      ...
      const { error } = await supabase
        ...
        .insert([{
          name: CONFIG_NAME,
          raw_text: "System Configuration for Gig Websites. Do not delete.",
          pdf_url: "",
          is_default: true,
          sections: websites
        }]);
    ...
    console.error("POST /api/gig-websites ERROR:", error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}
